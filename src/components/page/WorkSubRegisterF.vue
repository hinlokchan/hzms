<template>
  <div class="container">
	...
	v-if="subInfoForm">
      <span class="work-title-name">房地产子项目信息 <el-tag :type="newButtonType(registerStatus)" plain size="medium">{{newButtonValue(registerStatus)}}</el-tag></span>
	...
	>
		...
		<el-row :gutter="20">
			...
			<el-col :span="12">
				<el-form-item label="所在小区" prop="szxq" class="blue-item red-item">
					<el-input v-model.trim="subInfoForm.szxq" style="width: 100%" clearable></el-input>
				</el-form-item>
			</el-col>
			<el-col :span="24">
				<el-form-item label="详细地址" prop="xxdz" class="blue-item red-item">
					<el-input v-model.trim="subInfoForm.xxdz" style="width: 100%" clearable></el-input>
				</el-form-item>
			</el-col>
			<el-col :span="12">
		...
		<el-row :gutter="20">
			...
			<el-col :span="12">
				<el-form-item label="价值类型" prop="jzlx" class="blue-item">
					<el-select	v-model="subInfoForm.jzlx"  style="width: 100%;">
						<el-option
							v-for="item, index in jzlxOption"
							:key="index"
							:label="item"
							:value="item"
						>
			...
			<el-col :span="12">
				<el-form-item label="估价方法说明" prop="pgffsm" class="blue-item">
					<el-input v-model="subInfoForm.pgffsm" style="width: 100%" clearable
		...
		<el-row :gutter="20">
			<el-col :span="8">
				<el-form-item label="房屋用途" prop="fwyt" class="blue-item red-item">
					<el-select	v-model="subInfoForm.fwyt"  style="width: 100%;">
						<el-option
							v-for="item, index in fwytOption"
							:key="index"
							:label="item"
							:value="item"
						>
			...
			<el-col :span="8">
				<el-form-item label="房屋类型" prop="fwlx" class="blue-item red-item">
					<el-select	v-model="subInfoForm.fwlx"  style="width: 100%;">
						<el-option
							v-for="item, index in fwlxOption"
							:key="index"
							:label="item"
							:value="item"
						>
			...
			<el-col :span="8">
				<el-form-item label="房屋性质" prop="fwxz" class="blue-item red-item">
					<el-select	v-model="subInfoForm.fwxz"  style="width: 100%;">
						<el-option
							v-for="item, index in fwxzOption"
							:key="index"
							:label="item"
							:value="item"
						>
			...
			<el-col :span="8">
				<el-form-item label="户型结构" prop="hxjg" class="blue-item red-item">
					<el-select	v-model="subInfoForm.hxjg"  style="width: 100%;">
						<el-option
							v-for="item, index in hxjgOption"
							:key="index"
							:label="item"
							:value="item"
						>
			...
			<el-col :span="8">
				<el-form-item label="户型" prop="hx" class="blue-item red-item">
					<el-select	v-model="subInfoForm.hx"  style="width: 100%;">
						<el-option
							v-for="item, index in hxOption"
							:key="index"
							:label="item"
							:value="item"
						>
			...
			<el-col :span="8">
				<el-form-item label="朝向" prop="cx" class="blue-item red-item">
					<el-select	v-model="subInfoForm.cx"  style="width: 100%;">
						<el-option
							v-for="item, index in cxOption"
							:key="index"
							:label="item"
							:value="item"
						>
			...
			<el-col :span="8">
				<el-form-item label="所在楼层" prop="szlc" class="blue-item">
					<el-input v-model="subInfoForm.szlc" style="width: 100%" clearable
					oninput="value=value.replace(/[^\d]/g,'')"
					placeholder="请输入"></el-input>
				</el-form-item>
			</el-col>
			<el-col :span="8">
				<el-form-item label="装修程度" prop="zxcd" class="blue-item red-item">
					<el-select	v-model="subInfoForm.zxcd"  style="width: 100%;">
						<el-option
							v-for="item, index in zxcdOption"
							:key="index"
							:label="item"
							:value="item"
						>
			...
			<el-col :span="8">
				<el-form-item label="剩余使用年限" prop="sysynxc" class="blue-item">
					<el-input v-model="subInfoForm.sysynxc" style="width: 100%" clearable
					oninput="value=value.replace(/[^\d.]/g,'')"
					placeholder="请输入"></el-input>
				</el-form-item>
			</el-col>
		...
		<el-row :gutter="20">	
			<el-col :span="8">
				<el-form-item label="土地面积" prop="tdmj" class="blue-item">
					<el-input v-model="subInfoForm.tdmj" style="width: 100%" clearable
					placeholder="请输入(㎡)"
					oninput="value=value.replace(/[^\-\d.]/g,'')"
					@change="handleChangetdzjz"></el-input>
				</el-form-item>
			</el-col>
			<el-col :span="8">
				<el-form-item label="土地评估单价" prop="tdpgdj" class="blue-item">
					<el-input v-model="subInfoForm.tdpgdj" style="width: 100%" clearable
					placeholder="请输入(元/㎡)"
					oninput="value=value.replace(/[^\-\d.]/g,'')"
					@change="handleChangetdzjz"></el-input>
					<span v-if="subInfoForm.tdpgdj">
						{{changeMoneyToChinese(subInfoForm.tdpgdj)}}
					</span>
				</el-form-item>
			</el-col>
			<el-col :span="8">
				<el-form-item label="土地总价值" prop="tdzjz" class="blue-item">
					<el-input v-model="subInfoForm.tdzjz" style="width: 100%" disabled></el-input>
					<span v-if="subInfoForm.tdzjz">
						{{changeMoneyToChinese(subInfoForm.tdzjz)}}
					</span>
				</el-form-item>
			</el-col>		
		</el-row>
		<el-row :gutter="20">	
			<el-col :span="8">
				<el-form-item label="建筑面积" prop="jzmj" class="blue-item">
					<el-input v-model="subInfoForm.jzmj" style="width: 100%" clearable
					placeholder="请输入(㎡)"
					oninput="value=value.replace(/[^\-\d.]/g,'')"
					@change="handleChangejzzjz"></el-input>
				</el-form-item>
			</el-col>
			<el-col :span="8">
				<el-form-item label="建筑评估单价" prop="jzpgdj" class="blue-item">
					<el-input v-model="subInfoForm.jzpgdj" style="width: 100%" clearable
					placeholder="请输入(元/㎡)"
					oninput="value=value.replace(/[^\-\d.]/g,'')"
					@change="handleChangejzzjz"></el-input>
					<span v-if="subInfoForm.jzpgdj">
						{{changeMoneyToChinese(subInfoForm.jzpgdj)}}
					</span>
				</el-form-item>
			</el-col>
			<el-col :span="8">
				<el-form-item label="建筑总价值" prop="jzzjz" class="blue-item">
					<el-input v-model="subInfoForm.jzzjz" style="width: 100%" disabled></el-input>
					<span v-if="subInfoForm.jzzjz">
						{{changeMoneyToChinese(subInfoForm.jzzjz)}}
					</span>
				</el-form-item>
			</el-col>			
		</el-row>
...
export default {
	...
	data() {
		return {
			...
			subInfoRules:{
				...
				//房产
				szxq:[{ required: true, message: '请输入所在小区', trigger: 'blur' }],
				xxdz:[{ required: true, message: '请输入详细地址', trigger: 'blur' }],
				
				fwyt:[{ required: true, message: '请选择房屋用途', trigger: 'blur' }],
				fwlx:[{ required: true, message: '请选择房屋类型', trigger: 'blur' }],
				fwxz:[{ required: true, message: '请选择房屋性质', trigger: 'blur' }],
				hxjg:[{ required: true, message: '请选择户型结构', trigger: 'blur' }],
				hx:[{ required: true, message: '请选择户型', trigger: 'blur' }],
				cx:[{ required: true, message: '请选择朝向', trigger: 'blur' }],
				szlc:[{ required: true, message: '请输入所在楼层', trigger: 'blur' }],				
				zxcd:[{ required: true, message: '请选择装修程度', trigger: 'blur' }],
				sysynxc:[
				...
				jzpgdj:[
					{ required: true, message: '请输入建筑评估单价', trigger: 'blur' },
					{
						validator: (rule, value, callback) => {
							if(value){
								if (/^(([1-9]{1}\d*)|(0{1}))(\.\d{0,2})?$/.test(value) == false) {
			...
			//房产信息选项
			jzlxOption:['成交价格','正常价格','市场价格','评估价值','市场价值','投资价值','现状价值','快速变现价值','残余价值','抵押价值','抵押净值','法定优先受偿款','计税价值','保险价值','完全产权价值','无租约限制价值','出租人权益价值','承租人权益价值','建筑物价值','土地价值','楼面地价'],
			fwytOption:['住宅','宗教','园林绿化','工业、交通、仓储','别墅','涉外','医疗卫生','文化、娱乐、体育','公共运输','新闻','娱乐','监狱','集体宿舍','成套住宅','商业服务','旅游','军事','体育','物管用房','铁路','非成套住宅','工业','文化','仓储','电讯信息','教育','教育、医疗、卫生、可研','民航','高档公寓','科研','公共设施','商业、金融、信息','航运','金融保险','其他','办公','经营'],
			fwlxOption:['车库','别墅、高档公寓','仓储用房','住宅','其他','办公用房','商业用房','工业用房','房地价值'],
			fwxzOption:['其他','经济适用房','配套商品房','建筑物价值','市场化商品房','动迁房','公共租赁住房','廉租住房','限价普通商品住房','集资建房','定销商品房','福利房'],
			hxjgOption:['错层','复式楼','土地价值','跃层','平层','其他'],
			hxOption:['一居室','二居室','三居室','四居室','五居室','其他'],
			cxOption:['东','南','西','北','东南','东北','西南','西北','东南双面','西南双面','南北通透双面','其他通透双面','其他非通透双面','其他'],
			zxcdOption:['粗装修','毛坯','精装修'],
			 ...
			 */
			//评估目的
			//资产regEvalGoalOption:['IPO上市','公司制改建','对外投资','接受投资','合并、分立、破产、清算、解散','股东股权比例变动','产权转让','上市公司发行股份购买资产','资产转让、置换、拍卖','资产抵押/质押','资产捐赠','资产租赁','资产补偿/损失补偿','资产偿债','资产涉讼','资产收购','接受抵债资产','债转股','债务重组及其他','追溯评估','复核报告','了解价值','财务会计报告目的','计税价格评估','认定报关价格','其他'],
			...
			//项目分类
			cdChargeTypeOption:['房地产类','资产类','土地类','会计审计','咨询评价'],
	...
	methods: {
								...
								this.getSubProjectData(projId, subProjId, (spData)=>{
									this.getWorkAssignmentData(projId, subProjId, (waData)=>{
										...
										if(dpData){
											...
											//项目类型
											this.subInfoForm.cdChargeType = '房地产类'
		...
		//改变土地总价值
		handleChangetdzjz(){
			if(this.subInfoForm.tdmj && this.subInfoForm.tdpgdj){
				//改变土地总价值
				var fees = this.subInfoForm.tdmj*this.subInfoForm.tdpgdj;
				this.subInfoForm.tdzjz = fees.toFixed(2);
