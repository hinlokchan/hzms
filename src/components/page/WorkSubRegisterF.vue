...
export default {
	...
	data() {
		return {
			...
			subInfoRules:{
				...
				//房产
				evalObjCommunity:[{ required: true, message: '请输入所在小区', trigger: 'blur' }],
				evalObjAddress:[{ required: true, message: '请输入详细地址', trigger: 'blur' }],
				
				unitUsage:[{ required: true, message: '请选择房屋用途', trigger: 'blur' }],
				unitType:[{ required: true, message: '请选择房屋类型', trigger: 'blur' }],
				unitProperty:[{ required: true, message: '请选择房屋性质', trigger: 'blur' }],
				houseTypeStructure:[{ required: true, message: '请选择户型结构', trigger: 'blur' }],
				houseType:[{ required: true, message: '请选择户型', trigger: 'blur' }],
				towards:[{ required: true, message: '请选择朝向', trigger: 'blur' }],
				buildingNum:[{ required: true, message: '请输入所在楼层', trigger: 'blur' }],				
				decoDegree:[{ required: true, message: '请选择装修程度', trigger: 'blur' }],
				remainTerm:[
			...
			//房产信息选项
			valueTypeOption:['成交价格','正常价格','市场价格','评估价值','市场价值','投资价值','现状价值','快速变现价值','残余价值','抵押价值','抵押净值','法定优先受偿款','计税价值','保险价值','完全产权价值','无租约限制价值','出租人权益价值','承租人权益价值','建筑物价值','土地价值','楼面地价'],
			unitUsageOption:['住宅','宗教','园林绿化','工业、交通、仓储','别墅','涉外','医疗卫生','文化、娱乐、体育','公共运输','新闻','娱乐','监狱','集体宿舍','成套住宅','商业服务','旅游','军事','体育','物管用房','铁路','非成套住宅','工业','文化','仓储','电讯信息','教育','教育、医疗、卫生、可研','民航','高档公寓','科研','公共设施','商业、金融、信息','航运','金融保险','其他','办公','经营'],
			unitTypeOption:['车库','别墅、高档公寓','仓储用房','住宅','其他','办公用房','商业用房','工业用房','房地价值'],
			unitPropertyOption:['其他','经济适用房','配套商品房','建筑物价值','市场化商品房','动迁房','公共租赁住房','廉租住房','限价普通商品住房','集资建房','定销商品房','福利房'],
			houseTypeStructureOption:['错层','复式楼','土地价值','跃层','平层','其他'],
	...
	methods: {
		...
		handleChangeBuildingTotalValue(){
			if(this.subInfoForm.projTotalArea && this.subInfoForm.buildingAssemUnitPrice){
				//改变土地总价值
